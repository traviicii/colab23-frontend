...
export default function UserProfile() {
    ...
    const showInterests = () => {
        return user.data.interests?.map((interest) => <button className="white-button w-auto p-2 h-10 m-1 text-base whitespace-nowrap rounded-md border-grey border-2">{interest}</button>)
    }

    const showAvailability = () => {
        return user.data.availability?.map((availability) => <li>{availability}</li>)
    }
                    ...
                    <div className='middle flex w-10/12' >
                        ...
                        <div className='w-1/3 pt-12 pl-12 pr-12 pb-12'>
                            ...
                            <div>
                                ...
                                <ul style={{ listStyleType: 'disc', marginLeft: '25px' }}>
                                    <li>{user.data.prev_role}, {user.data.prev_exp}</li>
                                </ul>
                            </div>
                            <div>
                        ...
                        <div className='flex justify-center w-1/4 pt-12 px-6'>
                            <div className='message w-full mt-6'>
                                {/* <button onClick={() => window.location = `mailto:${user.data.email}`} className='border rounded w-full py-4 mb-4 font-bold rounded' style={{ backgroundColor: '#d7f7f9', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    <div className="bottom bg-white flex justify-center w-full py-8 ">
                        <div className="w-10/12 flex mb-20 mt-10" style={{ minHeight: '250px' }}>
                            ...
                            <div className="py-12 px-6 w-96" style={{ backgroundColor: '#f3d187' }}>
                                ...
                                {user.data.current_project_id ? <p><Link to={`/project-profile/${user.data.current_project_id}`} className='mt-4 font-bold underline'>{user.project ? user.project.name : ''}</Link> <br></br> - Project Admin: {user.project ? user.project.admin_name : ''}</p> : `${member.first_name} isn't involved in a project currently!`}
