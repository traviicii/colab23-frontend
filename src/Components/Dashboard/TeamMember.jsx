...
export default function TeamMember( {member} ) {
    ...
    return (
        <div className="w-1/4 bg-white p-4 rounded-lg shadow-lg text-center mx-4" >
            ...
            <button onClick={() => window.location = `mailto:${member.email}`} className='rounded-lg border border-rose-500 w-full py-4 mb-4 font-bold rounded' style={{display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <p className="text-left mt-8">Location: <b>{member.location}</b></p>
            <p className="text-left">Timezone: <b>{member.timezone}</b></p>
            <h3 className="text-left font-bold mt-8">Experience</h3>
            <ul className="text-left">
                <li>{member.prev_role ? member.prev_role : ''}</li>
                <li>{member.prod_role ? member.prod_role : ''}, {member.prod_exp ? member.prod_exp : ''}</li>
            </ul>
            <h3 className="text-left flex justify-between items-center mt-8"><b>About</b> <p className='text-xs'>(scrollable)</p></h3>
